...
bool RTree::RemoveRectRec(Rect* a_rect, const vector<pair<int, int>>& a_id, Node* a_node, ListNode** a_listNode)
{
        ...
        for (int index = 0; index < a_node->m_count; ++index)
        {
            if (Overlap(a_rect, &(a_node->m_branch[index].m_rect)))
            {
                if (!RemoveRectRec(a_rect, a_id, a_node->m_branch[index].m_child, a_listNode))
                {
                    ...
                    {
                        // Actualizar el rectángulo para que cubra correctamente los hijos
                        a_node->m_branch[index].m_rect = NodeCover(a_node->m_branch[index].m_child);
                    }
                    else
                    {
                        // Reinsertar los hijos si el nodo tiene muy pocos elementos
    ...
    else // Nodo hoja
    {
        for (int index = 0; index < a_node->m_count; ++index)
        {
            // Comparar los datos exactos para asegurar que eliminamos el elemento correcto
    ...
    if (node->IsLeaf()) {
        for (int i = 0; i < node->m_count; ++i) {
            if (Overlap2(&rect, &(node->m_branch[i].m_rect))) {
                if (!node->m_branch[i].m_data.empty()) {
                    for (const auto& point : node->m_branch[i].m_data) {
                        if (point.first >= rect.m_min[0] && point.first <= rect.m_max[0] &&
                            point.second >= rect.m_min[1] && point.second <= rect.m_max[1]) {
...
void RTree::guardarMBRsEnCSV(const char* archivo) {
    std::ofstream archivoCSV(archivo);
    if (!archivoCSV.is_open()) {
        std::cerr << "Error al abrir el archivo " << archivo << std::endl;
        return;
    }

    archivoCSV << "xmin,ymin,xmax,ymax,nivel" << std::endl; // Encabezado actualizado

    std::vector<std::vector<std::vector<std::pair<int, int>>>> mbrs;
    if (!getMBRs(mbrs)) {
        std::cerr << "Error al obtener los MBRs" << std::endl;
        archivoCSV.close();
        return;
    }

    for (size_t nivel = 0; nivel < mbrs.size(); ++nivel) {
        const auto& lista_mbrs = mbrs[nivel];
        for (const auto& mbr : lista_mbrs) {
            if (mbr.size() >= 2) {
                archivoCSV << mbr[0].first << "," << mbr[0].second << ","
                           << mbr[1].first << "," << mbr[1].second << ","
                           << nivel << std::endl;
            }
        }
    }
