...
public:
	...
	std::vector<std::pair<int, int>> Search(const Rect& rect);
	void Search(Node* node, const Rect& rect, std::vector<std::pair<int, int>>& results);

	void guardarMBRsEnCSV(const char* archivo);

	void SavePointsToCSV(Node* node, std::ofstream& file)
	{
		if (node->IsInternalNode())
		{
			for (int i = 0; i < node->m_count; ++i)
			{
				SavePointsToCSV(node->m_branch[i].m_child, file);
			}
		}
		else // Nodo hoja
		{
			for (int i = 0; i < node->m_count; ++i)
			{
				for (const auto& punto : node->m_branch[i].m_data) {
	...
	void ExportTreeToCSV(Node* root)
	{
		std::ofstream file("puntos.csv");
		if (!file.is_open())
		{
			std::cerr << "Error al abrir puntos.csv\n";
			return;
		}
	
		file << "x,y\n"; // encabezado CSV
		SavePointsToCSV(root, file);
		file.close();
		std::cout << "Puntos exportados a puntos.csv\n";
	}
	void ExportTreeToCSV()
	{
		ExportTreeToCSV(m_root);
	}
...
protected:
  ...
  float CalcRectArea(Rect* a_rect);
	...
	bool Overlap2(const Rect* a_rectA, const Rect* a_rectB) const;
